/* Problem: "The four adjacent digits in this 1000-digit number that have the greatest product are 9 × 9 × 8 × 9 = 5832.
 ...
 */
// Goal: Convert the number into an array with 1000 values (in the same order of course.) Find the 13 adjacent values that have the largest product (the 13 largest adjacent values, no matter what.) Print out the product.
...
function finalProduct (digits, consecutive){
    var r = 0, out = 0, start = 0, end = consecutive, size = digits.length - consecutive;
    for (var i = 0; i < size; i++){
        var d = digits.slice(start + i, end + i);
        var ld = d[end - 1];
        if (ld === 0){
            i += consecutive -1;
            continue;
        ...
        if (ld > out){
            ...
            if (p > r){
                console.log("The new highest consecutive digits are " + d);
                r = p;
            }
        }
        out = d[0];
...
console.log(finalProduct(digits, 13));

// This elegant code is sadly not mine.  I attempted to have the 1000 digit number split into an array of strings, one per digit.  Then I tried to convert that array of strings to an array of digits.  If that worked, my plan would have been similar to the finalProduct function, albeit less elegant.  Sadly, the function that would have converted the number to an array couldn't handle 1000 digits.